...
app.get("/v1/points/get/:id", requireParamFields(["id"]), async (req, res) => {
  try {
    ...
    logger.error(
      `Error while getting point by id: ${colorText(e.message, "red")}`
    );
...
app.get(
  "/v1/points/get-by-organid/:organid",
  requireParamFields(["organid"]),
  async (req, res) => {
    try {
      ...
      logger.error(
        `Error while getting points by organ id: ${colorText(e.message, "red")}`
      );
      res.status(500).send({ error: e.message });
    }
  }
