...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (
      file.mimetype === "image/openslide" ||
      [".png", ".jpeg", ".jpg", ".svs"].includes(
...
async function processOrgan(organ, name, targetPath) {
  try {
    logger.info(
      `Organ ${colorText("processing", "yellow")}: ID=${
        organ.id
      }, name="${name}"`
    );
    ...
    await convertSvsToTilesExternal(
      targetPath,
      `${organTilesDir}/${organ.id}`,
      organ.id
    );
    ...
    logger.info(
      `Organ ${colorText("processed", "green")}: ID=${organ.id}, name="${name}"`
    );
    ...
    logger.error(
      `Error processing organ ID=${organ.id}: ${colorText(
        error.message,
        "red"
      )}`
    );
    ...
    worker.on("message", (message) => {
      if (message.type === "log") {
        logger[message.level](
          `Worker for organ ID=${organId}: ${message.message}`
        );
    ...
    try {
      ...
      if (!name || !categoryid || !file) {
        ...
        return res
          .status(400)
          .send({
            error: "Missing required fields: name, categoryid, or file",
          });
      }

      const synonym = transliterate(name, {
        lowercase: true,
        separator: "_",
      }).replace(/[^a-z0-9_]/g, "");
      ...
      const organ = await OrganModel.create({
        name,
        categoryid,
        synonym,
        status: "PROCESSING",
