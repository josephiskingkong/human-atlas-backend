...
app.get("/v1/organs/get/:id", requireParamFields(["id"]), async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      name: organ.name,
      details: organ.detailedDescription,
    ...
    logger.error(
      `Error while getting organ by id: ${colorText(e.message, "red")}`
    );
...
app.get(
  "/v1/organs/get-by-categoryid/:categoryid",
  ...
  async (req, res) => {
    try {
      ...
      logger.error(
        `Error while getting organs by category id: ${colorText(
          e.message,
          "red"
        )}`
      );
...
app.get("/v1/organs/get-all-done/", async (req, res) => {
  try {
    const organs = await OrganModel.findAll({ where: { status: "DONE" } });
