...
app.get("/v1/tests", async (req, res) => {
    try {
        ...
        logger.info(`Retrieved ${tests.length} tests${categoryId ? ` for category ID=${categoryId}` : ''}`);

        return res.status(200).json(tests.map(test => ({
            id: test.id,
            title: test.title,
            categoryId: test.categoryId,
            categoryName: test.category?.name || 'Без категории',
 ...
 */
app.get("/v1/tests/:testId/questions", authenticateToken, requireParamFields(['testId']), async (req, res) => {
    try {
        ...
        logger.info(`Retrieved ${questions.length} questions for test ID=${testId}`);
        
        return res.status(200).json(questions);
    } catch (e) {
        logger.error(`Error retrieving questions: ${colorText(e.message, 'red')}`);
        res.status(500).json({ error: e.message });
    }
});
...
app.get("/v1/tests/results/my", authenticateToken, async (req, res) => {
    try {
        ...
        logger.info(`Retrieved ${results.length} test results for user ID=${userId}`);
        ...
        logger.error(`Error retrieving test results: ${colorText(e.message, 'red')}`);
        res.status(500).json({ error: e.message });
    }
});
 ...
 */
app.get("/v1/tests/results/:resultId", authenticateToken, requireParamFields(['resultId']), async (req, res) => {
    try {
        ...
        logger.error(`Error retrieving test result: ${colorText(e.message, 'red')}`);
        res.status(500).json({ error: e.message });
    }
});