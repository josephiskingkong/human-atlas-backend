 ...
 */
app.post("/v1/tests/add", authenticateToken, requireBodyFields(['title', 'categoryId', 'duration']), async (req, res) => {
    ...
    try {
        ...
        logger.info(`Test ${colorText('created', 'green')}: ID=${test.id}, title="${title}"`);
 ...
 */
app.post("/v1/tests/:testId/questions/add", authenticateToken, 
    ...
    async (req, res) => {
        ...
        try {
            ...
            await TestModel.increment('questionCount', { by: 1, where: { id: testId }, transaction });

            await transaction.commit();
            
            logger.info(`Question ${colorText('added', 'green')} to test ID=${testId}: question ID=${question.id}`);
