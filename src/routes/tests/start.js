...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      logger.info(
        `Started test session: User ID=${userId}, Test ID=${testId}, Session ID=${session.id}`
      );
 ...
 */
app.get(
  "/v1/tests/sessions/:sessionId",
  authenticateToken,
  requireParamFields(["sessionId"]),
  async (req, res) => {
    try {
      ...
      logger.error(
        `Error checking session status: ${colorText(e.message, "red")}`
      );
      res.status(500).json({ error: e.message });
    }
  }
